...
def connect(ip_address, port1, username, password, database, values, identifier_type,update_type):
    returnId=0;
    if update_type == "patient": 
       returnId = connectPatient(ip_address, port1, username, password, database, values, identifier_type,update_type)
    elif update_type == "order":
       returnId = connectOrder(ip_address, port1, username, password, database, values, identifier_type,update_type) 
    elif update_type == "order_line":
       returnId = connectOrderLine(ip_address, port1, username, password, database, values, identifier_type,update_type) 
    return returnId 

def connectPatient(ip_address, port1, username, password, database, values, identifier_type,update_type):
    port1 = int(port1)
...
def connectOrder(ip_address, port1, username, password, database, values, identifier_type,update_type):
    port1 = int(port1)
    connMRS = MySQLdb.connect (host = ip_address,
        port=port1,
        user=username,
        passwd = password,
        db=database)
    # Map ids values of OpenERP to OpenMRS order payment table

    reference = values['order_ref']
    orderedBy= values['ordered_by']
    patient =  values['patient_ref']
    paymentDate= values['order_payment_date']
    orderstate = values['order_state']
    ...
    for item in values:
     ...
     # Map ids values of OpenERP to OpenMRS order payment table
    cursoromrs1 = connMRS.cursor()
    inspersonstmnt = """INSERT INTO order_payments(order_ref,order_type,payment_date,payment_status,\
                  cleared_by,patient_ref,payment_notes) \
                  VALUES(%s, %s,%s,%s, '1',%s,'%s')"""
    cursoromrs1.execute(inspersonstmnt, (reference,orderType,paymentDate,orderstate,patient, paymentNotes))
    paymentId = int(cursoromrs1.lastrowid)
    cursoromrs1.execute("commit")
    cursoromrs1.close()

    #insert into prescrption openmrs dispense events order lines
...
def connect_write(ip_address, port1, username, password, database, patientid, values, identifier_type,update_type):
    returnId=0;
    if update_type == "patient": 
       returnId = connect_writePatient(ip_address, port1, username, password, database, values, identifier_type,update_type)
    elif update_type == "order":
       returnId = connect_writeOrder(ip_address, port1, username, password, database, values, identifier_type,update_type) 
    elif update_type == "order_line":
       returnId = connect_writeOrderLine(ip_address, port1, username, password, database, values, identifier_type,update_type) 
    return returnId 

def connect_writePatient(ip_address, port1, username, password, database, values, identifier_type,update_type):
...
def connect_writeOrder(ip_address, port1, username, password, database, values, identifier_type,update_type):
    port1 = int(port1)
    connMRS = MySQLdb.connect (host = ip_address,
        port=port1,
        user=username,
